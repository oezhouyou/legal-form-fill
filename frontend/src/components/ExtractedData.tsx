...
function Section({
  ...
  return (
    <div className="mb-6">
      <h3 className="text-sm font-semibold text-gray-800 uppercase tracking-wider mb-3 pb-2 border-b">
        {title}
      </h3>
...
export default function ExtractedData({ data, warnings, onChange }: Props) {
  const updateAttorney = (field: string, value: string) => {
    onChange({
      ...data,
      attorney: { ...data.attorney, [field]: value },
    });
  };
  ...
  const updateEligibility = (field: string, value: string) => {
    onChange({
      ...data,
      eligibility: { ...data.eligibility, [field]: value },
    });
  };
  ...
  const updatePassport = (field: string, value: string) => {
    onChange({
      ...data,
      passport: { ...data.passport, [field]: value },
    });
  };
  ...
  return (
    <div>
      ...
      <Section title="Attorney / Representative (Part 1)">
        ...
        <Field label="State (2-letter)" value={data.attorney.state} onChange={(v) => updateAttorney("state", v)} />
      ...
      <Section title="Eligibility (Part 2)">
      ...
      <Section title="Passport / Beneficiary (Part 3)">
        ...
        <Field label="Date of Birth (YYYY-MM-DD)" value={data.passport.date_of_birth} onChange={(v) => updatePassport("date_of_birth", v)} />
        <Field label="Place of Birth" value={data.passport.place_of_birth} onChange={(v) => updatePassport("place_of_birth", v)} />
        <Field label="Sex (M/F/X)" value={data.passport.sex ?? ""} onChange={(v) => updatePassport("sex", v)} />
        <Field label="Issue Date (YYYY-MM-DD)" value={data.passport.issue_date} onChange={(v) => updatePassport("issue_date", v)} />
        <Field label="Expiry Date (YYYY-MM-DD)" value={data.passport.expiry_date} onChange={(v) => updatePassport("expiry_date", v)} />
      </Section>
    </div>
  );
}
